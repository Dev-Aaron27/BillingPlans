...
class SubscriptionSuspended
{
    public static function getTemplate(array $data): string
    {
        if (
            !isset($data['app_name'], $data['app_url'], $data['first_name'],
                ...
                $data['app_support_url'], $data['plan_name'], $data['credits_required'])
        ) {
            return '';
        }

        $raw = MailTemplate::getByName('billingplans_subscription_suspended')['body'] ?? '';
        ...
        return self::parseTemplate($raw, [
            ...
            'credits_required' => (string) $data['credits_required'],
        ]);
    }
    ...
    public static function parseTemplate(string $template, array $data): string
    {
        ...
        $template = str_replace('{credits_required}', $data['credits_required'], $template);
    ...
    public static function send(array $data): void
    {
        if (
            !isset($data['uuid'], $data['email'], $data['subject'],
                ...
                $data['plan_name'], $data['credits_required'], $data['enabled'])
        ) {
            return;
        }
